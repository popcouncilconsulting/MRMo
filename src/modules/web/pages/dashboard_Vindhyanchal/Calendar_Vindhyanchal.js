...
//Actions
import {
  GetEventsVindhyanchal,
  UpdateEventsVindhyanchal,
  GetEquipmentsVindhyanchal,
  UpdateEquipmentsVindhyanchal,
  GetPeople,
  UpdatePeople,
} 
from "../../../../helpers/db_vindhyanchal";
//Styles
...
class Dnd extends Component {
  ...
  componentDidMount() {
    ...
    GetEventsVindhyanchal(this.props.uid).then(querySnapshot => {
    ...
    GetEquipmentsVindhyanchal(this.props.uid).then(querySnapshot => {
      querySnapshot.forEach(doc => {
  ...
  moveEvent({event, start, end}) {
    ...
    if (idx > -1) {
      if(this.props.uid === event.ownerId){
        ...
        UpdateEventsVindhyanchal(event.id).update({start, end}).then(
          this.setState({
            events: nextEvents,
          })
        ).catch(error => {
      ...
      if(this.props.uid === event.ownerId){
        ...
        UpdateEventsVindhyanchal(newEventId).set(updatedEvent).then(
          this.setState({
            events: nextEvents,
          })
        ).catch(error => {
  ...
  createEquipment = ({title, desc, room}) => {
    ...
    UpdateEquipmentsVindhyanchal(newEquipmentId).set(updatedEquipment).then(
      this.setState({
  ...
  editEvent = ({id, title, desc, room}) => {
    ...
    UpdateEventsVindhyanchal(id).update({title, desc, room}).then(
      this.setState({
        events: nextEvents,
      })
    ).catch(error => {
  ...
  editEquipment = ({id, title, desc, room}) => {
    ...
    UpdateEquipmentsVindhyanchal(id).update({title, desc, room}).then(
      this.setState({
        equipments: nextEquipments,
      })
    ).catch(error => {
  ...
  deleteEvent = ({id}) => {
    ...
    UpdateEventsVindhyanchal(id).delete().then(
      this.setState({
        events: nextEvents,
      })
    ).catch(error => {
  ...
  deleteEquipment = ({id}) => {
    ...
    UpdateEquipmentsVindhyanchal(id).delete().then(
      this.setState({
        equipments: nextEquipments,
      })
    ).catch(error => {
    ...
    if (this.state.events) {
      return (
        <div className={'row'}>
          ...
          <h3 style={{textAlign: 'center', color: '#03A9F4'}} >Vindhyanchal (NOIDA)</h3>
            ...
            <Dialog title="Update Meeting For - Vindhyanchal (Noida)"
                    modal={false}
            ...
            <Dialog title="Create Meeting For - Vindhyanchal (Noida)"
                    modal={false}
