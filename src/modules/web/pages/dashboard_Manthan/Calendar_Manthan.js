...
import {
  GetEventsManthan,
  UpdateEventsManthan,
  GetEquipmentsManthan,
  UpdateEquipmentsManthan,
  GetPeople,
  UpdatePeople,
} 
from "../../../../helpers/db_manthan";
//Styles
...
class Dnd extends Component {
  ...
  componentDidMount() {
    ...
    GetEventsManthan(this.props.uid).then(querySnapshot => {
    ...
    GetEquipmentsManthan(this.props.uid).then(querySnapshot => {
      querySnapshot.forEach(doc => {
  ...
  moveEvent({event, start, end}) {
    ...
    if (idx > -1) {
      if(this.props.uid === event.ownerId){
        ...
        UpdateEventsManthan(event.id).update({start, end}).then(
          this.setState({
            events: nextEvents,
          })
        ).catch(error => {
      ...
      if(this.props.uid === event.ownerId){
        ...
        UpdateEventsManthan(newEventId).set(updatedEvent).then(
          this.setState({
            events: nextEvents,
          })
        ).catch(error => {
  ...
  createEquipment = ({title, desc, room}) => {
    ...
    UpdateEquipmentsManthan(newEquipmentId).set(updatedEquipment).then(
      this.setState({
  ...
  editEvent = ({id, title, desc, room}) => {
    ...
    UpdateEventsManthan(id).update({title, desc, room}).then(
      this.setState({
        events: nextEvents,
      })
    ).catch(error => {
  ...
  editEquipment = ({id, title, desc, room}) => {
    ...
    UpdateEquipmentsManthan(id).update({title, desc, room}).then(
      this.setState({
        equipments: nextEquipments,
      })
    ).catch(error => {
  ...
  deleteEvent = ({id}) => {
    ...
    UpdateEventsManthan(id).delete().then(
      this.setState({
        events: nextEvents,
      })
    ).catch(error => {
  ...
  deleteEquipment = ({id}) => {
    ...
    UpdateEquipmentsManthan(id).delete().then(
      this.setState({
        equipments: nextEquipments,
      })
    ).catch(error => {
      ...
      return (
        <div className={'row'}>
          ...
          <div style={{height: 500, width: 1100}} className={'col-10'}>
            ...
            <h3 style={{textAlign: 'center', color: '#03A9F4'}} >Manthan(IHC)</h3>
            ...
            <Dialog title="Update Meeting For - Manthan (IHC)"
                    modal={false}
            ...
            <Dialog title="Create Meeting For - Manthan (IHC)"
                    modal={false}
